...
import { el, icon, fmtSize, fmtTime, toast } from './util.js';
...
export function mountWorkbench(root, { onImport = () => {} } = {}) {
  ...
  const download = (file) => {
    saveFile(file, file.name).catch((error) => toast('导出失败：' + errorMessage(error)));
  };
  const status = () => {
  ...
  const proxyCard = card('02 / 生成转录音频');
  ...
  const useAudio = button('生成 16 kHz 音频', { main: true, onPick: async () => {
    ...
    } catch (err) { if (!disposed) proxyNote.textContent = controller.signal.aborted ? '已取消' : errorMessage(err, '音频转换失败，请重试'); }
  ...
  const convert = button('提取聆听音频', { main: true, onPick: async () => {
    ...
    try {
      ...
      if (!disposed) conversionNote.textContent = controller.signal.aborted ? '已取消转换' : errorMessage(err, '音频提取失败，请检查文件后重试');
    } finally { conversion = null; if (!disposed) update(); }
  } });
  ...
  const fields = [
    ['接口地址', 'asrBaseUrl', { placeholder: 'https://api.example.com/v1' }],
    ['API Key', 'asrApiKey', { secret: true, placeholder: '无须验证时留空' }],
    ['模型', 'asrModel', { placeholder: 'whisper-1' }],
    ['语言代码', 'asrLanguage', { placeholder: '如 ja、en、zh；留空自动识别' }],
  ...
  const wordRow = switchRow('词级时间戳', '需要转录模型支持',
  ...
  function validateParams(persist = true) {
    ...
    try {
      ...
      extraError = '';
    } catch (err) { extraError = errorMessage(err, '自定义参数无效，请检查后重试'); }
  ...
  const start = button('开始转录', { main: true, onPick: async () => {
    ...
    clearResult();
    audioNote.textContent = '正在转录…'; update();
    try {
      ...
      if (disposed) return;
      importNote.textContent = hasSubtitle() ? ''
        : '转录结果缺少时间戳，请选择详细 JSON、SRT 或 VTT 格式重新转录。';
    } catch (err) {
      if (!disposed) audioNote.textContent = controller.signal.aborted ? '已取消转录' : errorMessage(err, '转录失败，请重试');
  ...
  asrCard.append(el('p', 'field-hint', '转录时将上传音频至所配置的服务。'),
  ...
  const addAudio = button('添加音频到首页', { onPick: async () => {
    ...
    } catch (err) { if (!disposed) audioImportNote.textContent = '添加失败：' + errorMessage(err); }
  ...
  targetField.append(el('span', 'field-label', '字幕保存位置'));
  const target = button('', { onPick: async () => {
    if (targetOpen) return;
    const list = el('div', 'workbench-track-list');
    list.append(el('p', 'pane-note', '正在加载媒体列表…'));
    openSheet('选择媒体', list, { cls: 'sheet-tall', onClose: () => {
    ...
    if (!loaded) {
      list.append(el('p', 'pane-note', '媒体列表加载失败，请重新打开。'));
      return;
    }
    if (!targetTracks.length) list.append(el('p', 'pane-note', '暂无媒体，请先在首页导入。'));
  ...
  target.setAttribute('aria-label', '字幕保存位置'); target.setAttribute('aria-haspopup', 'dialog');
  const targetCopy = el('span', 'workbench-entry-copy'), targetName = el('b', null, '选择媒体');
  const targetHint = el('span');
  ...
  async function refreshTargets(preferred = target.value) {
    ...
    try {
      ...
      update();
      return true;
    } catch (err) { if (!disposed) importNote.textContent = errorMessage(err, '媒体列表加载失败，请重试'); }
  }
  const add = button('导入字幕', { main: true, onPick: async () => {
    ...
    try {
      ...
      try {
        ...
        if (!response.track || !Array.isArray(response.track.sentences)) throw new Error('未收到字幕分析结果，请重试');
        await saveAnalysis(record.id, response.track, { transcriptName: subtitle.name, transcriptFile: subtitle });
        if (!disposed) importNote.textContent = '字幕已导入并完成分析';
      } catch (err) {
        const message = errorMessage(err, '字幕分析失败，请在播放页重试');
        if (!disposed) importNote.textContent = '字幕已保存。' + message;
        try { await patchTrack(record.id, { error: message }); }
        catch (saveError) { if (!disposed) toast(errorMessage(saveError, '无法保存分析状态，请重试')); }
      }
      await onImport();
    } catch (err) { if (!disposed) importNote.textContent = '导入失败：' + errorMessage(err); }
  ...
  subtitleImport.append(targetField, languageRow,
    group(switchRow('导入后分析字幕', '生成分词与注音',
  ...
  function update() {
    ...
    targetName.textContent = selected?.title || '选择媒体';
    targetName.title = selected?.title || '';
    targetHint.textContent = selected ? `${fmtTime(selected.duration)} · ${selected.transcript ? '已有字幕' : '待导入字幕'}` : '';
    targetHint.hidden = !selected;
