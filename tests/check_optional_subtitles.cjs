...
(async () => {
  ...
  for (const browserType of [chromium]) {
    ...
    try {
      ...
      assert.equal(await page.getByRole('button', { name: /^视频字幕字号/ }).count(), 0);
      ...
      assert.equal(await page.locator('.font-preview-text').first().evaluate((n) => getComputedStyle(n).fontSize), '32px');
      ...
      for (const viewport of [{ width: 844, height: 390 }, { width: 1440, height: 900 }]) {
        ...
        await page.evaluate(() => document.getElementById('btnToolSettings').click());
        await page.getByRole('button', { name: /^系统字幕字号/ }).click();
        const localCaption = page.getByRole('spinbutton', { name: '系统字幕字号 (px)' });
        await localCaption.fill('30'); await localCaption.press('Tab');
        assert.equal(await page.evaluate(async () => (await import('/js/trackcfg.js')).trackCfg.video.captionSize), 30);
        await page.locator('.sheet-head [aria-label="关闭"]').click();
        await page.evaluate(() => document.getElementById('btnToolSettings').click());
        await page.getByRole('button', { name: /^字幕管理/ }).click();
