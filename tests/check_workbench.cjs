...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.getByRole('button', { name: '导入字幕', exact: true }).isDisabled(), true);
    ...
    await page.getByLabel('API Key', { exact: true }).fill('smoke-key');
    ...
    await page.getByText('字幕已导入', { exact: true }).waitFor();
    ...
    await page.getByText('转录完成 ·', { exact: false }).waitFor();
    await page.getByRole('button', { name: '导入字幕', exact: true }).click();
    await page.getByText('字幕已导入并完成分析', { exact: true }).waitFor();
    assert.equal(await page.getByRole('button', { name: '导入字幕', exact: true }).isDisabled(), true);
    ...
    await page.getByLabel('字幕保存位置', { exact: true }).click();
    await page.locator('.sheet.is-open').waitFor();
    ...
    await page.getByRole('button', { name: '导入字幕', exact: true }).click();
    await page.getByText('字幕已保存。分析服务暂时不可用', { exact: false }).waitFor();
    ...
    await page.getByLabel('字幕保存位置', { exact: true }).click();
    await page.waitForFunction(() => document.querySelector('button[aria-label="字幕保存位置"]').value === '');
    await page.getByRole('button', { name: '关闭', exact: true }).click();
    assert.equal(await page.getByRole('button', { name: '导入字幕', exact: true }).isDisabled(), true);
    ...
    assert.equal(await page.getByLabel('接口地址', { exact: false }).inputValue(), base + '/v1');
    assert.equal(await page.getByLabel('API Key', { exact: true }).inputValue(), 'smoke-key');
